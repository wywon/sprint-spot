...
/* ── 주차 관련 기록 가려내기 ──────────────────────────────────
   [b13] 관리자 '주차 관리' 화면의 「센서 감지 · 변경 로그」 카드가
   대시보드 '최근 변경' 과 같은 목록을 필터 없이 쓰고 있었다.
   그래서 주차와 무관한 기록까지 그 카드에 올라왔다.
     T06 정리 완료 처리            ← 홀 운영 이야기
     11:30 이서준님 예약 착석 처리  ← 예약 이야기
   카드 부제가 "센서가 보고한 변화와 손으로 지정한 기록이에요" 인데
   테이블·예약 기록이 섞이면 그 설명이 거짓말이 된다.

   ★ 왜 문구로 가르는가
     ActivityLog 에는 분류 칼럼이 없다. 칼럼을 늘리면 마이그레이션이 필요하고
     팀원 전원이 로컬 DB 를 다시 말아야 한다. 시연을 앞두고 치를 값이 아니다.
     대신 로그를 쓰는 일곱 곳의 문구를 전부 확인하고 가르는 기준을 잡았다.

     주차 쪽 (남긴다)
       P5 주차 중 → 주차 가능              api/detect · who 센서
       주차면 8곳이 한꺼번에 …              api/detect · who 센서
       센서 연결이 돌아왔어요                api/detect · who 시스템
       P3 수동 지정 → 주차 가능             api/admin/slots
       P3 센서 감지로 복귀                  주차 화면
       주차장 배치도 수정 · 10면            주차 화면 · api/admin/layout
     그 외 (걸러낸다)
       T06 정리 완료 / T06 사용 중 → 빈 자리  api/admin/tables
       12:00 홍길동님 예약 착석 처리          api/admin/reservations
       매장 정보 수정 · 매장명 · 업종         api/admin/stores

     매장 수정 라벨(매장명·업종·주소·전화·영업시간…)에 '주차'나 '센서' 가
     들어가는 것이 없어 오분류가 나지 않는다. 라벨을 늘릴 때 이 점을 확인할 것.
     테이블과 주차면 배치도를 한 번에 저장하면 한 줄에 둘 다 적히는데,
     그건 주차면도 바뀐 것이 맞으므로 여기 올라오는 게 옳다.

   나중에 ActivityLog 에 kind 칼럼이 생기면 이 함수 안쪽만 바꾸면 된다. */
export function isParkingLog(l: { who: string; msg: string }): boolean {
  // 센서가 남기는 기록은 전부 주차면 이야기다
  if (l.who === '센서') return true;
  // 'P3 …' 처럼 주차면 코드로 시작하는 줄
  if (/^P\d+\b/.test(l.msg)) return true;
  // '주차장 배치도 수정' · '센서 게이트웨이 응답 없음'
  return /주차|센서/.test(l.msg);
}
